...
async def reset(dut):
    # mimic the initial condition when a devices is powered or manually reset
    # Instances of simulation models for specific signals or interfaces
    # These are the defined inputs (captured from top_module.v)
    dut.enc0_a = 0  
    dut.enc0_b = 0
    dut.enc1_a = 0
    dut.enc1_b = 0
    ...
    await ClockCycles(dut.clk, 5) # ensuring the system reaches a stable reset state
    dut.reset <= 0
    await ClockCycles(dut.clk, 5) # how long to wait for the debouncers to clear

async def run_encoder_test(encoder, dut_enc, max_count): # automates the testing of an encoder (counts to a specific value)
    for i in range(clocks_per_phase * 2 * max_count):  # generates a sequence of encoder updates over multiple cycles
        await encoder.update(1)

    # let noisy transition finish, otherwise can get an extra count
    for i in range(10): # runs 10 cycles
        await encoder.update(0)
    
    assert(dut_enc == max_count) # checks if the encoder count matches the max_count expected after the test sequence

@cocotb.test() # cocotb test function, encoder system with PWM outputs
